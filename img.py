...
def parse_args():
    ...
    parser.add_argument('--human_weight', type=str, default=None)
    parser.add_argument('--human_pose_weight', type=str, default=None)
...
@torch.no_grad()
def run(args):
    human_weight, human_pose_weight, device = args.human_weight, args.human_pose_weight, args.device
    ...
    efficient_pose = EfficientPose(
        folder_path=r"/content/bin_imgs", model_name="III")

    # Load CNN
    model_CNN = modules.config_CNN(human_pose_weight, device)

    # Load tracker / deepsort
    NN_BUDGET = None
    MAX_COSINE_DISTANCE = 0.3
    TRACKING_MODEL = r"/content/gdrive/MyDrive/Turnstiles_Fare_Evasion_Python/models/deep_sort/mars-small128.pb"
    ...
    for index, (path, im, im0s, vid_cap, _) in enumerate(dataset):
        ...
        yolo_preds = non_max_suppression(yolo_preds, 0.3, 0.3, [0], None, max_det=100)[0]
        t4 = time_sync()
        # nms time for yolo
        ...
        for *xyxy, conf, cls in yolo_preds:
            ...
            image = efficient_pose.estimatePose(img, 'bin.jpg')
            image = cv2.resize(image, (80, 80)) / 255.0
            image = image[:, :, ::-1] # BGR to RGB
            image = image.transpose(2, 0, 1).reshape(1, 3, 80, 80) # (C,H,W)
            image = np.ascontiguousarray(image)
            image = torch.from_numpy(image).to(device).type(torch.FloatTensor)
            out = model_CNN(image)
            label = None
        ...
        bbox_preds = tlbr2tlwh(bbox_preds)
        ...
        for track in tracker.tracks:
            ...
            print(track.pre_state, track.current_state)
            ...
            if ((not FORWARD_FLOW and track.pre_state == 1 and track.current_state == -1) \
                ...
                h_end = min(int(track_box[3]) + offset, im0s.shape[0])
                w_start = max(0, int(track_box[0]) - offset)
                w_end = min(int(track_box[2]) + offset, im0s.shape[1])
                temp_img = im0s[h_start: h_end, w_start: w_end, :]
                label = efficient_pose.estimatePose(temp_img, 'bin.jpg')
                print(label)
                cv2.rectangle(im0s, (int(track_box[0]), int(track_box[1])),
